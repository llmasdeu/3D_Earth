...
uniform sampler2D u_texture_normal;                                         // new
...
void main(void)
{
    ...
    vec3 texture_normal = texture2D(u_texture_normal, v_uvs).xyz;           // new
    vec3 texture_spec = texture2D(u_texture_spec, v_uvs).xyz;              // new
    vec3 ambient_color = texture_color * u_ambient;
    vec3 N = normalize(v_normal);
    vec3 L = normalize(u_light_dir);
    vec3 N_orig = N;                                                        // new
    N = perturbNormal(N, v_world_vertex_pos, v_uvs, texture_normal);        // new
    N = mix(N_orig, N, 1.0);                                                // new
    ...
    vec3 specular_color = u_light_color * RdotE * texture_spec;             // new
